...
export default function MyBits() {
    ...
    const handleBuyout = () => {
        alert(`Toode ${rentalItem.name} välja ostetud! (SQL: UPDATE rentals SET status='müüdud' WHERE rental_id=${id})`);
    };

    const handleReturn = () => {
        alert(`Tagastusprotsess algatatud! (SQL: UPDATE rentals SET status='tagastatud' WHERE rental_id=${id})`);
    };
    return (
        <>
...
const styles = StyleSheet.create({
    ...
    mainTitle: { color: '#FFFFFF', fontSize: 22, fontWeight: 'bold', textAlign: 'center', marginBottom: 40 },
    statusCard: { backgroundColor: '#EAEAEA', borderRadius: 20, width: '100%', paddingVertical: 20, alignItems: 'center', justifyContent: 'center', marginBottom: 40 },
